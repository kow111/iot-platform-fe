  ...
  const onFinish = async (values: any) => {
    try {
      ...
      localStorage.setItem(
        "role",
        JSON.stringify(response.data.data.user.roles)
      );
      ...
      toast.error(
        (error as any).response.data.message[0] || "Đăng nhập thất bại!"
      );
      console.error("Login failed:", error);
      if (
        (error as any).response.data.message[0] === "This user is not verified"
      ) {
